...
const { apiToCommanderMap, defaultValues } = require('@src/commands/smapi/customizations/parameters-map');
...
const _makeOption = (param) => {
    ...
    const requiredTemplateValue = defaultValue && defaultValue.inWorkspace ? '[OPTIONAL IN WORKSPACE]' : requiredTemplate[param.required];
    return {
        name: `${alias}--${paramName} <${paramName}>`,
        description:
        (`${requiredTemplateValue} ${(param.description || '').trim()} \n`
        + `${arrayTemplate}`
        + `${jsonTemplate[json]}`
        + `${enumTemplate} `).trim(),
        defaultValue: shouldMakeOptional ? defaultValue.value : undefined
    };
};
/**
...
module.exports = { makeSmapiCommander, defaultValues };
