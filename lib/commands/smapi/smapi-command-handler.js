...
const { apiToCommanderMap, customizationMap, defaultValues } = require('@src/commands/smapi/customizations/parameters-map');
...
const _showCheckStatusHint = (locationHeader, profile) => {
    ...
    const pathKeys = Object.keys(modelIntrospector.modelJson.paths);
    let swaggerMatch;
    let statusCheckHintCommand;
    let parameters = {};
    for (const pathKey of pathKeys) {
        swaggerMatch = _matchSwaggerPattern(baseUrl, pathKey);
...
const _addSkillIdFromWorkspace = (values, profile) => {
    if (values.skillId && values.skillId === defaultValues.get('skillId').value) {
        try {
            new ResourcesConfig(path.join(process.cwd(), CONSTANTS.FILE_PATH.ASK_RESOURCES_JSON_CONFIG));
            values.skillId = ResourcesConfig.getInstance().getSkillId(profile);
        } catch (err) {
            if (err instanceof CliFileNotFoundError) {
                console.log("error: required option '-s,--skill-id <skill-id>' not specified. the option is required outside of a skill workspace.");
                process.exit(1);
            } else {
                throw err;
            }
...
const smapiCommandHandler = (swaggerApiOperationName, flatParamsMap, commanderToApiCustomizationMap, inputCmdObj, modelIntrospector) => {
    ...
    _addSkillIdFromWorkspace(inputOpts, profile);
