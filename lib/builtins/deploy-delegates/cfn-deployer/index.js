...
async function invoke(reporter, options, callback) {
    ...
    try {
        const deployResult = await _deploy(reporter, options, deployProgress);

        deployProgress.endpoint = { uri: deployResult.endpointUri };
        deployProgress.resultMessage = _makeSuccessMessage(deployResult.endpointUri, alexaRegion);
...
async function _deploy(reporter, options, deployProgress) {
    ...
    const templatePath = _getTemplatePath(alexaRegion, userConfig);
    ...
    const deployRequest = await helper.deployStack(stackId, stackName, templatePath, stackParameters, capabilities);
...
function _getTemplatePath(alexaRegion, userConfig) {
