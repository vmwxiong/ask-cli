...
module.exports = class SkillMetadataController {
 ...
 please make sure the skill metadata deployment has succeeded with result of a valid skillId.`);
    ...
    /**
     * Read from the ./skill-package/interactionModel/custom directory, check what locales the current project
    ...
    getInteractionModelLocales() {
        ...
        const validFileList = fs.readdirSync(iModelFolderPath).filter((file) => {
            const fileExt = path.extname(file);
            const fileNameNoExt = path.basename(file, fileExt);
            return fileExt !== '.json' || !R.includes(fileNameNoExt, CONSTANTS.ALEXA.LANGUAGES);
        });
        const result = {};
